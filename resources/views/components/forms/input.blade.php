...
<div>
    ...
    @else
        <input name="{{$name}}" id="{{$id}}" {{$attributes->class(['border-1 border-input-border shadow-input-shadow rounded-10 py-10px px-12px w-full h-normal-input focus:border-input-focus placeholder:text-input-placeholder placeholder:font-semibold'])->merge(['type' => 'text'])}} data-role="tagsinput" />
    @endif
    @error($name)
