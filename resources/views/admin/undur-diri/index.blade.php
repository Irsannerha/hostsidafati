        ...
        <div class="card-box mb-30">
          <div class="pd-20">
            ...
            @elseif(Auth::user()->role == 'akademik')
              <a href="{{ route('akademik.undur-diri.create') }}" class="btn btn-primary font-weight-bold"><i class="fa fa-plus"></i> Tambah Data</a>
            @endif
          </div>
          <div class="pb-20">
            <table class="data-table table stripe hover nowrap">
              ...
              <tbody>
              @foreach ($groupedUndurDiri as $udri => $items)
                @php
                $tahun = $items->first()->tahun;
                $totalTs = $totals->firstWhere('ts_id', $udri);
                @endphp
                <tr>
                        ...
                        <div class="modal-dialog modal-lg modals modal-dialog-scrollable" role="document">
                          <div class="modal-content" data-bgcolor="#d0d0d0">
                            <div class="modal-header">
                              <h4 class="modal-title" id="myLargeModalLabel"><i class="fa fa-paperclip" aria-hidden="true"></i> Mahasiswa Mengundurkan Diri TS {{ $tahun->ts }}</h4>
                                            ...
                                            @if($undur->ts_id == $udri)
                                                ...
                                                <tr>
                                                  ...
                                                  <a href="{{ route('superadmin.undur-diri.edit', $undur->id ) }}" class="btn btn-xxs btn-primary mr-1" style="border-radius: 15px; padding: 0.2rem 0.5rem; font-size: 0.9rem;" data-color="#fff">
                                                    @elseif(Auth::user()->role == 'akademik')
                                                    <a href="{{ route('akademik.undur-diri.edit', $undur->id ) }}" class="btn btn-xxs btn-primary mr-1" style="border-radius: 15px; padding: 0.2rem 0.5rem; font-size: 0.9rem;" data-color="#fff">
                                                  @endif
                ...
                <div class="pd-20 card-box card-hdr">
                    <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <a href="{{ asset('assets/templateImport/template_UndurDiri.xlsx') }}" class="btn btn-primary" download>
                                <i class="fa fa-download"></i> Download
                            </a>
                        </div>
                        <div class="form-group">
                            <form action="{{ route('UndurDiri.import') }}" method="POST" enctype="multipart/form-data">
                                @csrf
 ...
 <!-- End Import Modal -->
  ...
  <script>
    function exportToExcel() {
        window.location.href = "{{ url('UndurDiri/export') }}";
    }
</script>
  <style>
    .modals {
