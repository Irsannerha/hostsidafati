        ...
        <div class="card-box mb-30">
          <div class="pd-20">
            ...
            @if (Auth::user()->role == 'superadmin')
              <a href="{{ route('superadmin.wafat.create') }}" class="btn btn-primary font-weight-bold"><i class="fa fa-plus"></i> Tambah Data</a>
            @elseif (Auth::user()->role == 'akademik')
              <a href="{{ route('akademik.wafat.create') }}" class="btn btn-primary font-weight-bold"><i class="fa fa-plus"></i> Tambah Data</a>
            @endif
          </div>
          <div class="pb-20">
            <table class="data-table table stripe hover nowrap">
              ...
              <tbody>
                @foreach ($groupedWafat as $died => $items)
                @php
                $tahun = $items->first()->tahun;
                $totalTs = $totals->firstWhere('ts_id', $died);
                @endphp
                <tr>
                  ...
                  <td>
                    <div class="dropdown">
                      <a class="btn btn-xxs btn-primary mr-1" style="border-radius: 15px; padding: 0.2rem 0.5rem; font-size: 0.9rem;" data-color="#fff" data-toggle="modal" data-toggle="modal" data-target="#showModal{{ $died }}"><i class="dw dw-eye"></i> Detail </a>
                      <!-- Ini Modal Mahasiswa Undur diri -->
                      <div class="modal fade bs-example-modal-lg" id="showModal{{ $died }}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-lg modals modal-dialog-scrollable" role="document">
                          <div class="modal-content" data-bgcolor="#d0d0d0">
                            <div class="modal-header">
                              <h4 class="modal-title" id="myLargeModalLabel"><i class="fa fa-paperclip" aria-hidden="true"></i> Mahasiswa Wafat TS {{ $tahun->ts }}</h4>
                              ...
                              <div class="pd-10 card-box card-hdr">
                                <div class="scrollable-table">
                                    <table class="table table-striped">
                                      ...
                                      <tbody>
                                        @php
                                            $grand_total_mhs_wafat = 0;
                                            $total_mhs_wafat_genap = 0;
                                            $total_mhs_wafat_ganjil = 0;
                                            $index = 0;
                                        @endphp

                                        @foreach($wafat as $rips)
                                            @if($rips->ts_id == $died)
                                                @php
                                                    $index++;
                                                    $total_mhs_wafat = $rips->mhs_wafat_genap + $rips->mhs_wafat_ganjil;
                                                    $grand_total_mhs_wafat += $total_mhs_wafat;
                                                    $total_mhs_wafat_genap += $rips->mhs_wafat_genap;
                                                    $total_mhs_wafat_ganjil += $rips->mhs_wafat_ganjil;
                                                @endphp
                                                <tr>
                                                    <td>{{ $index }}</td>
                                                    <td class="font-weight-bold">{{ $rips->prodi->prodi }}</td>
                                                    <td>{{ $rips->mhs_wafat_genap }}</td>
                                                    <td>{{ $rips->mhs_wafat_ganjil }}</td>
                                                    <td>{{ $total_mhs_wafat }}</td>
                                                    <td>
                                                    @if (Auth::user()->role == 'superadmin')
                                                      <a href="{{ route('superadmin.wafat.edit', $rips->id ) }}" class="btn btn-xxs btn-primary mr-1" style="border-radius: 15px; padding: 0.2rem 0.5rem; font-size: 0.9rem;" data-color="#fff">
                                                    @elseif (Auth::user()->role == 'akademik')
                                                      <a href="{{ route('akademik.wafat.edit', $rips->id ) }}" class="btn btn-xxs btn-primary mr-1" style="border-radius: 15px; padding: 0.2rem 0.5rem; font-size: 0.9rem;" data-color="#fff">
                                                    @endif
                                                    <i class="icon-copy dw dw-edit2"></i> Edit
                                                  </a>
                                                  <a class="btn btn-xxs btn-primary mr-1" style="border-radius: 15px; padding: 0.2rem 0.5rem; font-size: 0.9rem;" data-color="#fff" data-toggle="modal" data-target="#deleteModal{{ $rips->id }}">
                                                    <i class="icon-copy dw dw-delete-3"></i> Hapus
                                                  </a>
                                                  @include('admin.wafat.delete', ['rips' => $rips])
                                                </td>
                                                </tr>
                                            @endif
                                        @endforeach
                                        ...
                                        <tr>
                                            ...
                                            <td><strong class="badge badge-success btn-sm">{{ $total_mhs_wafat_genap }}</strong></td>
                                            <td><strong class="badge badge-success btn-sm">{{ $total_mhs_wafat_ganjil }}</strong></td>
                                            <td><strong class="badge badge-success btn-sm">{{ $grand_total_mhs_wafat }}</strong></td>
                                            <td></td>
                                        </tr>
                                    </tbody>
                                  </table>
                ...
                <div class="pd-20 card-box card-hdr">
                    <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <a href="{{ asset('assets/templateImport/template_Wafat.xlsx') }}" class="btn btn-primary" download>
                                <i class="fa fa-download"></i> Download
                            </a>
                        </div>
                        <div class="form-group">
                            <form action="{{ route('Wafat.import') }}" method="POST" enctype="multipart/form-data">
                                @csrf
 ...
 <!-- End Import Modal -->
  ...
  <script>
    function exportToExcel() {
        window.location.href = "{{ url('Wafat/export') }}";
    }
</script>
  <style>
    .modals {
