...
<!-- Modal Lihat -->
<div class="modal fade" id="showModal{{ $ta->id }}" tabindex="-1" role="dialog" aria-labelledby="showModalLabel{{ $ta->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
        <div class="modal-content" data-bgcolor="#d0d0d0">
            <div class="modal-header">
                <h5 class="modal-title" id="showModalLabel{{ $ta->id }}"><i class="fa fa-paperclip" aria-hidden="true"></i> Detail Data Pengajuan Tugas Akhir</h5>
            ...
            <div class="modal-body table-responsive">
                <div class="pd-20 card-box card-hdr" data-bgcolor="#fff">
                    <dl class="text-content-box">
                        <dt>Program Studi</dt>
                        <dd class="custom-dd">{{ $ta->Prodi ? $ta->Prodi->prodi : '' }}</dd>

                        <dt>Nama</dt>
                        <dd class="custom-dd">{{ $ta->nama }}</dd>

                        <dt>NIM</dt>
                        <dd class="custom-dd">{{ $ta->nim }}</dd>

                        <dt>Keperluan</dt>
                        <dd class="custom-dd">
                            @if($ta->keperluan == 'Permohonan Izin Penelitian Tugas Akhir')
                            <span class="badge badge-success" style="border-radius: 10px; padding: 0.2rem 1.2rem; font-size: 15px;">Permohonan Izin Penelitian Tugas Akhir</span>
                            @else
                            <span class="badge badge-danger" style="border-radius: 10px; padding: 0.2rem 1.2rem; font-size: 15px;">Permohonan Data Tugas Akhir</span>
                            @endif
                        </dd>

                        <dt>Instansi</dt>
                        <dd class="custom-dd">{{ $ta->instansi }}</dd>

                        <dt>Alamat Instansi</dt>
                        <dd class="custom-dd">{{ $ta->alamat_instansi }}</dd>

                        <dt>Tujuan Jabatan Pimpinan</dt>
                        <dd class="custom-dd">{{ $ta->tjp }}</dd>

                        <dt>Pelaksanaan</dt>
                        <dd class="custom-dd">
                            @if($ta->pelaksanaan == 'Offline')  
                            <span class="badge badge-success" style="border-radius: 10px; padding: 0.2rem 1.2rem; font-size: 15px;">Offline</span>
                            @else
                            <span class="badge badge-danger" style="border-radius: 10px; padding: 0.2rem 1.2rem; font-size: 15px;">Online</span>
                            @endif
                        </dd>
                        ...
                        <dt>No. HP</dt>
                        <dd class="custom-dd">{{ $ta->no_hp }}</dd>

                        <dt>Email</dt>
                        <dd class="custom-dd">{{ $ta->email }}</dd>

                        <dt>Nama Pembimbing Satu</dt>
                        <dd class="custom-dd">{{ $ta->nama_pembimbing_satu }}</dd>

                        <dt>Nama Pembimbing Dua</dt>
                        <dd class="custom-dd">{{ $ta->nama_pembimbing_dua }}</dd>

                        <dt>Judul</dt>
                        <dd class="custom-dd">{{ $ta->judul }}</dd>
